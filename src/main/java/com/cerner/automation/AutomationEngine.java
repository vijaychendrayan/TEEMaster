    ...
    public void executeTask()throws InterruptedException, NullPointerException,IOException,ArrayIndexOutOfBoundsException, ParserConfigurationException{
        ...
        try{
            ...
            while (rowIterator.hasNext()){
              ...
              /*  Ratnadeep: 4/6/2018 :  Reading match value from different sheet
                System.out.println("test point :" + processData.match.length() +"Param 2:"+ processData.param.length()+"param3 :"+processData.active);
              //  if(processData.match.length() > 1 && processData.param.length()>1 && processData.active=="A") {
                    System.out.println("test point 1");
                    if (processData.match.substring(0, 2).equals("S:")) {
                        String[] cellInfoArray;
                        cellInfoArray = processData.match.split(":");
                        String sheetName = cellInfoArray[1];
                        int cellRow = Integer.parseInt(cellInfoArray[2]);
                        int cellCol = Integer.parseInt(cellInfoArray[3]);
                        System.out.println("Match Sheet " + sheetName + " row " + cellRow + " col " + cellCol);
                        processData.match = excelWorkbook.getSheet(sheetName).getRow(cellRow).getCell(cellCol).getStringCellValue().toString();
                        System.out.println("Cell Value for match ===> " + processData.match);
                    }

                    if (processData.param.substring(0, 2).equals("S:")) {
                    String[] cellInfoArray1;
                    cellInfoArray1 = processData.param.split(":");
                    String sheetName1 = cellInfoArray1[1];
                    int cellRow1 = Integer.parseInt(cellInfoArray1[2]);
                    int cellCol1 = Integer.parseInt(cellInfoArray1[3]);
                    System.out.println("Match Sheet "+sheetName1+" row "+cellRow1+" col "+cellCol1);
                    processData.param = excelWorkbook.getSheet(sheetName1).getRow(cellRow1).getCell(cellCol1).getStringCellValue().toString();
                    System.out.println("Cell Value for parameter ===> "+processData.param);
                    }
               // }
                //Ratnadeep -Change  End*/
                    ...
                    while (itr.hasNext()) {
                        ...
                        if (pdtmp.active.equals("A")){
                            ...
                            if(pdtmp.param.substring(0,2).equals("S:")){
                                ...
                                pdtmpSheet.param = excelWorkbook.getSheet(sheetName).getRow(cellRow).getCell(cellCol).getStringCellValue().toString();
                                System.out.println("Cell Value ===> "+pdtmp.param);
                            }

                            //Ratnadeep: 4/6/2018 :  Reading match value from different sheet

                            if(pdtmp.match.substring(0,2).equals("S:")){
                                String[] cellInfoArray1;
                                cellInfoArray1 = pdtmp.match.split(":");
                                String sheetName1 = cellInfoArray1[1];
                                int cellRow1 = Integer.parseInt(cellInfoArray1[2]);
                                int cellCol1 = Integer.parseInt(cellInfoArray1[3]);
                                System.out.println("Match Sheet "+sheetName1+" row "+cellRow1+" col "+cellCol1);
                                pdtmpSheet.match = excelWorkbook.getSheet(sheetName1).getRow(cellRow1).getCell(cellCol1).getStringCellValue().toString();
                                System.out.println("Match Cell Value Inside For  ===> "+pdtmpSheet.match);
                            }
                            //Ratnadeep -Change  End*/
